  ...
  return (
    <AppShell>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-indigo-400/10 px-2 py-0.5 text-xs text-indigo-300">
                      {result.match_type}
                    </span>
                    {result.paper.topics.slice(0, 2).map((t) => (
                  ...
                  <p className="mt-1 text-sm text-zinc-400 line-clamp-2">
                    {result.matched_chunk ?? result.paper.abstract}
                  </p>
                  <p className="mt-2 text-xs text-zinc-500">
                    {result.paper.authors.slice(0, 3).join(', ')}
                    {result.paper.authors.length > 3 ? ' et al.' : ''}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-2">
                  ...
                  <a
                    href={`https://arxiv.org/abs/${result.paper.arxiv_id}`}
                    target="_blank"
      ...
      {data && !isFetching && data.results.length === 0 && (
        <p className="mt-8 text-center text-zinc-500">No results found.</p>
      )}
    </AppShell>
  );
}
...
export default function SearchPage() {
  return (
    <Suspense
      fallback={
        <AppShell>
          <div className="mt-20 text-center text-zinc-500">Loading search…</div>
        </AppShell>
      }
    >
      <SearchContent />
