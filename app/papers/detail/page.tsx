...
function PaperDetailContent() {
  ...
  const { data: paper } = useQuery({ queryKey: ['paper', paperId], queryFn: () => v3.paper(paperId), enabled: paperId.length > 0 });
  const { data: related } = useQuery({ queryKey: ['related', paperId], queryFn: () => v3.related(paperId, 5), enabled: !!paper });
  ...
  if (!paperId) {
    return <AppShell><div className="text-center text-zinc-500 mt-20">Choose a paper from the papers page.</div></AppShell>;
  }
    ...
    <AppShell>
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="flex flex-col gap-4">
          ...
          {related && related.results.length > 0 && <Card><h2 className="font-medium mb-3">Related papers</h2><div className="space-y-3">{related.results.slice(0, 4).map((r) => <a key={r.paper.id} href={`/papers/detail?id=${encodeURIComponent(r.paper.id)}`} className="block rounded-xl bg-black/20 p-3 hover:bg-black/30 transition-colors"><p className="text-sm font-medium line-clamp-2">{r.paper.title}</p><p className="mt-1 text-xs text-zinc-500">{Math.round(r.score * 100)}% similar</p></a>)}</div></Card>}
        </div>
      </div>
    </AppShell>
  );
...
export default function PaperDetailPage() {
  return (
    <Suspense fallback={<AppShell><div className="mt-20 text-center text-zinc-500">Loading paper…</div></AppShell>}>
      <PaperDetailContent />
    </Suspense>
  );
}
